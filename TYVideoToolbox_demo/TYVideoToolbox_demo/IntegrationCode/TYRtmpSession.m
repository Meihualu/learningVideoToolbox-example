...
#import "NSMutableData+Buffer.m"
#import "TYRtmpConfig.h"

static const size_t kRTMPSignatureSize = 1536;

@interface TYRtmpSession()<TYStreamSessionDelegate>{
...
@property (nonatomic,strong) TYStreamSession *session;

@property (nonatomic,  copy) NSString *url;

@property (nonatomic,assign) TYRtmpSessionStatus rtmpStatus;
...
- (TYStreamSession *)session{
    if (_session == nil) {
        _session = [[TYStreamSession alloc] init];
...
- (void)setRtmpStatus:(TYRtmpSessionStatus)rtmpStatus{
...
- (void)setConfig:(TYRtmpConfig *)config{
    _config = config;
    self.url = config.url;
}
- (void)connect{
...
- (void)sendPacket:(NSData *)data :(RTMPChunk_0)metadata{
    ...
    TYFrame *frame = [[TYFrame alloc] init];
